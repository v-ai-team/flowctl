...
cmd_cursor_dispatch() {
  ...
  if [[ "$merge_only" == "true" ]]; then
    cmd_war_room merge
    echo -e "${CYAN}→ War Room merged. Tiếp tục chạy: ${BOLD}bash scripts/workflow.sh cursor-dispatch${NC}\n"
    return 0
  fi
  ...
  if [[ "$skip_war_room" == "false" ]]; then
    ...
    if [[ "$score" -ge "${WF_WAR_ROOM_THRESHOLD:-3}" ]]; then
      ...
      echo -e "  ${BOLD}bash scripts/workflow.sh cursor-dispatch --merge${NC} (merge war room outputs)"
      echo -e "  ${BOLD}bash scripts/workflow.sh cursor-dispatch --skip-war-room${NC} (bỏ qua, dispatch thẳng)\n"
      return 0
    else
